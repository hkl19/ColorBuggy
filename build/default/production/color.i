...
void convertHSV (struct storecolor *sc, struct storecolor *sh){
    ...
    switch(maxIndex)
    {
        case 0: h = 0;
        break;
        case 1: h = 60 *((sc->G[i]-sc->B[i])/(maxValue-minValue));
        break;
        case 2 : h = 60 *(2+((sc->B[i]-sc->R[i])/(maxValue-minValue)));
        break;
        case 3 : h = 60 *(4+((sc->R[i]-sc->G[i])/(maxValue-minValue)));
        break;
    }
