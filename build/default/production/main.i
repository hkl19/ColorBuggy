...
void main(void){
    ...
    struct motor motorR;
    struct motor motorL;
    struct storecolor sc;
    struct getcolor cr;
    struct getcolor clear;

    struct storecolor sh;
    struct getcolor hr;
    struct getcolor hh;

    unsigned int actArr[25];
    unsigned int fwdArr[25];

    char str[80];
    unsigned char lenArr = 25;
...
# 69 "main.c"
        ...
        if (LATDbits.LATD7 == 1 && !PORTFbits.RF3){
            mode = 2;
        }
    }
    ...
    while (mode==2){
            ...
            scanColor (&clear,&sc,&motorL,&motorR,&fwdTime);
